...
> =>
  Effect.gen(function* () {
    ...
    // Root and side turns can use different settings; restore only the compacted thread's context.
    const additionalContextByThread = new Map<
      string,
      CodexTurnStartParamsWithCollaborationMode["additionalContext"]
    >();
    ...
    const restoreAdditionalContext = (threadId: string) =>
      Effect.gen(function* () {
        const context = additionalContextByThread.get(threadId);
        if (!context) return;
    ...
    const handleRawNotification = (notification: CodexServerNotification) =>
      Effect.gen(function* () {
        ...
        if (notification.method === "thread/closed") {
          additionalContextByThread.delete(notification.params.threadId);
        }
    ...
    const interruptSideChat = (id: string) =>
      Effect.gen(function* () {
        ...
        // Release side-owned request handlers before interrupting the transport.
    ...
    const closeSideChat = (id: string) =>
      Effect.gen(function* () {
        ...
        sideChat = undefined;
        sideTurnId = undefined;
        additionalContextByThread.delete(id);
        yield* emitEvent({
          kind: "notification",
          threadId: options.threadId,
          method: "thread/closed",
    ...
    return {
      ...
      sendSideChat: (id, input) =>
        Effect.gen(function* () {
          ...
          const model = normalizeCodexModelSlug(input.model ?? current.model);
          const models = options.models ? yield* options.models : [];
          const modelName = models.find((entry) => entry.slug === model)?.name;
          ...
          additionalContextByThread.set(id, params.additionalContext);
      ...
      sendTurn: (input) =>
        Effect.gen(function* () {
          ...
          additionalContextByThread.set(providerThreadId, params.additionalContext);
