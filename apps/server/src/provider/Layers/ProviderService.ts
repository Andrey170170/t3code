...
const makeProviderService = Effect.fn("makeProviderService")(function* (
   ...
   *
   * Credentials carry browser and task capabilities independently. Every
   * adapter treats a missing session as "no MCP server"; each toolkit also
   * checks its capability before performing an operation.
   */
  /**
  ...
  const prepareMcpSession = (threadId: ThreadId, providerInstanceId: ProviderInstanceId) =>
    Effect.gen(function* () {
      ...
      const tasksEnabled = yield* Effect.gen(function* () {
        const settings = yield* serverSettings.getSettings;
        const instance = yield* registry.getInstanceInfo(providerInstanceId);
        return settings.enableAgentTaskAccess && instance.driverKind === "codex";
      }).pipe(Effect.catch(() => Effect.succeed(false)));
      const capabilities = [
        ...(browserEnabled ? (["preview"] as const) : []),
        ...(tasksEnabled ? (["tasks"] as const) : []),
      ];
  ...
  const sendTurn: ProviderServiceMethod<"sendTurn"> = Effect.fn("sendTurn")(function* (rawInput) {
    ...
    return yield* Effect.gen(function* () {
      ...
      if (input.agentOrigin && routed.adapter.provider !== "codex") {
        return yield* toValidationError(
          "ProviderService.sendTurn",
          "Agent messages require a Codex provider",
        );
      }
      if (
