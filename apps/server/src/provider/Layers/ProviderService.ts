...
const makeProviderService = Effect.fn("makeProviderService")(function* (
  ...
  const agentAccessSettings = Effect.fn("ProviderService.agentAccessSettings")(
    function* (threadId: ThreadId) {
      ...
      const environment = {
        ...
        tasks: settings.enableAgentTaskAccess,
      };
    ...
    Effect.catch((cause) =>
      Effect.logWarning(
        "Could not read server settings; withholding agent browser, device, and task access for this session.",
        { cause },
      ).pipe(Effect.as({ browser: false, device: false, tasks: false })),
    ),
  );
  ...
  const agentAccessCapabilities = Effect.fn("ProviderService.agentAccessCapabilities")(function* (
    ...
    if (access.tasks) {
      ...
      if (instance.driverKind === "codex") capabilities.add("tasks");
    }
    return capabilities;
  });
  ...
  const prepareMcpSession = (threadId: ThreadId, providerInstanceId: ProviderInstanceId) =>
    Effect.gen(function* () {
      const capabilities = yield* agentAccessCapabilities(threadId, providerInstanceId);
