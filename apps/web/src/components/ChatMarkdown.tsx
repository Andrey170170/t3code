...
import {
  ...
  CHAT_MARKDOWN_SANITIZE_SCHEMA as BASE_CHAT_MARKDOWN_SANITIZE_SCHEMA,
...
const CHAT_MARKDOWN_SANITIZE_SCHEMA = {
  ...BASE_CHAT_MARKDOWN_SANITIZE_SCHEMA,
  attributes: {
    ...BASE_CHAT_MARKDOWN_SANITIZE_SCHEMA.attributes,
    div: [
      ...(BASE_CHAT_MARKDOWN_SANITIZE_SCHEMA.attributes?.div ?? []),
      ...CODEX_ARTIFACT_TEMPLATE_HAST_PROPERTIES,
    ],
    img: [
      ...(BASE_CHAT_MARKDOWN_SANITIZE_SCHEMA.attributes?.img ?? []),
      "dataLocalSrc",
      "dataMarkdownTitle",
    ],
  },
  protocols: {
    ...BASE_CHAT_MARKDOWN_SANITIZE_SCHEMA.protocols,
    src: [...(BASE_CHAT_MARKDOWN_SANITIZE_SCHEMA.protocols?.src ?? []), "file"],
  },
} satisfies Parameters<typeof rehypeSanitize>[0];

const CHAT_MARKDOWN_REMARK_PLUGINS = [
  ...BASE_CHAT_MARKDOWN_REMARK_PLUGINS,
  remarkCodexDirectives,
...
function ChatMarkdownSignedImage(
...
const ChatMarkdownTemporaryImage = memo(function ChatMarkdownTemporaryImage(
  props: ChatMarkdownAssetImageProps,
) {
  ...
  return <ChatMarkdownSignedImage {...props} assetUrl={assetUrl} />;
});

/** Workspace capability is authoritative; temporary files are queried only after it fails. */
const ChatMarkdownWorkspaceImage = memo(function ChatMarkdownWorkspaceImage(
  props: ChatMarkdownAssetImageProps,
) {
  const workspaceAssetUrl = useAssetUrlState(props.threadRef.environmentId, {
    _tag: "workspace-file",
    threadId: props.threadRef.threadId,
    path: props.path,
  });
  if (workspaceAssetUrl._tag === "Failure") {
    return <ChatMarkdownTemporaryImage {...props} />;
  }
  return <ChatMarkdownSignedImage {...props} assetUrl={workspaceAssetUrl} />;
});
        ...
        if (managedAttachmentUrl) {
          return (
            <img
              ...
              {...expandableMarkdownImageProps(imageExpand, managedAttachmentUrl, altText)}
            />
          );
        }
