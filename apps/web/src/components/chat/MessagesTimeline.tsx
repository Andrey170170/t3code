...
// inside a message are exposed below this level. Ordinary author headings stay
// visually hidden; agent origins remain visible so forwarded messages are identifiable.
const MESSAGE_HEADING_LEVEL = 3;

function MessageAuthorHeading({
  children,
  visible = false,
}: {
  children: string;
  ...
  return (
    <h3
      className={cn(
        "select-none",
        visible ? "mb-2 text-xs font-medium text-muted-foreground" : "sr-only",
      )}
    >
      {children}
    </h3>
  ...
  return (
    <div className="group flex flex-col items-end gap-1">
      <div className="relative max-w-[80%] rounded-2xl bg-message p-3 text-message-foreground">
        <MessageAuthorHeading visible={Boolean(row.message.agentOrigin)}>
          {row.message.agentOrigin ? `Agent message · ${row.message.agentOrigin.threadId}` : "You"}
