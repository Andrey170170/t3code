...
describe("ChatMarkdown streaming", () => {
  it("preserves math before a cached code fence and completes math after it", async () => {
    vi.stubGlobal("IS_REACT_ACT_ENVIRONMENT", true);
    let renderer: ReactTestRenderer | undefined;
    const prefix = "\\(x^2\\)\n\n```text\nliteral \\(code\\)\n```\n\n";
    ...
    const annotations = () =>
      renderer!.root
        .findAll(
          (node) => typeof node.type === "string" && node.props.encoding === "application/x-tex",
        )
        .map(textContent);
    try {
      await act(async () => {
        renderer = create(<ChatMarkdown cwd="/tmp/project" text={prefix + "\\(y"} isStreaming />);
      });
      expect(annotations()).toEqual(["x^2"]);
      await act(async () => {
        renderer!.update(
          <ChatMarkdown cwd="/tmp/project" text={prefix + "\\(y+1\\)"} isStreaming />,
        );
      });
      expect(annotations()).toEqual(["x^2", "y+1"]);
      await act(async () => {
        renderer!.update(
          <ChatMarkdown cwd="/tmp/project" text={prefix + "\\(y+1\\)\n\n\\[z^2\\]"} />,
        );
      });
      expect(annotations()).toEqual(["x^2", "y+1", "z^2"]);
    } finally {
...
function renderMarkdown(markdown: string, lineBreaks = false): string {
  return renderToStaticMarkup(
    <ChatMarkdown cwd="/tmp/project" text={markdown} lineBreaks={lineBreaks} />,
  );
}

describe("chat Markdown math", () => {
  it("keeps single dollar prices and shell variables literal", () => {
    const html = renderMarkdown("Costs $5 or $10; use $HOME and $PATH.");
    expect(html).not.toContain('class="katex"');
    expect(html).toContain("Costs $5 or $10; use $HOME and $PATH.");
  });

  it("renders math when raw HTML parsing is disabled", () => {
    const html = renderToStaticMarkup(
      <ChatMarkdown cwd="/tmp/project" text={"Inline \\(x^2\\)."} parseRawHtml={false} />,
    );
    expect(html).toContain('encoding="application/x-tex">x^2</annotation>');
  });
